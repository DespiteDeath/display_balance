...
def Connect(name, balance):
    mydb = mysql.connector.connect(
      host="localhost",
      user="root",
      passwd="------",  # напиши пароль для бд
      database="mydatabase"
    )

    mycursor = mydb.cursor()                                             # создание бд
    # mycursor.execute("CREATE TABLE test_network (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), balance INT(255), "
    #                  "timestamp TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)")   # создание таблицы и столбцов
    ...
    Draw_plot(myresult, '%d-%m-%y %H:%M')



def Draw_plot(_myresult, date_format=None):                       # Вывод графика
    X = [i[3] for i in _myresult]
    Y = [i[2] for i in _myresult]
    ax = plt.subplot()
    ax.plot(X, Y)
